...
export class OrderService {
  ...
  protected convertDateFromClient(order: IOrder): IOrder {
    return Object.assign({}, order, {
      orderDate: order.orderDate?.isValid() ? order.orderDate.format(DATE_FORMAT) : undefined,
      shipDate: order.shipDate?.isValid() ? order.shipDate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((order: IOrder) => {
        ...
        order.shipDate = order.shipDate ? dayjs(order.shipDate) : undefined;
      });
    }
    return res;
  }
